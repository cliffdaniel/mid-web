...
const Arquitectura: React.FC = () => {
  ...
  const options = [
    { text: 'todos', index: 0 },
    { text: 'edificios', index: 1 },
    { text: 'viviendas', index: 2 },
    { text: 'gran escala', index: 3 },
  ];

  const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);
  const filteredImages = activeOption === 0
    ? projects
    : projects.filter(project => project.type === options[activeOption].text);
  ...
  return (
    <div className="relative gap-10 md:grid-cols-2 max-w-[1140px] mx-auto px-4 md:px-[35px]">
      <div ref={stickyRef} className={`sticky flex-col transition-height duration-300 ease-in-out items-center ${esSticky ? 'h-[150px]' : 'h-[50px]'} top-0 z-50 bg-[#F2F2F2] flex flex-col items-center gap-4 mb-4 md:flex-row md:justify-center mb-[30px]`}>
        {options.map(option => (
          <div
            key={option.index}
        ...
        {!project && (<ResponsiveMasonry columnsCountBreakPoints={{ 0: 1, 750: 2 }}>
          <Masonry gutter="20px">
            {filteredImages.map((item, index) => (
              ...
              >
                <img src={item.images[0]} alt={`Carousel ${index}`} className="w-full h-auto" />
                {hoveredIndex === index && (
